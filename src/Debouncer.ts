import { DebouncerInterface, GenericDebouncerContract } from "./Types";
import { DebouncedFunc } from "./DebouncedFunc";

export class Debouncer<Contract extends GenericDebouncerContract = GenericDebouncerContract>
  implements DebouncerInterface<Contract>
{
  ...
  public bounce<K extends keyof Contract = string>(
    ...
    if (!debouncedFunc || debouncedFunc.status !== "waiting") {
      debouncedFunc = new DebouncedFunc<Contract[K]>(typeof waitMs === "number" ? waitMs : this.defaultWaitMs);
